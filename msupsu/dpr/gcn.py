...
class GCN(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4, with_relu=True, with_bias=True, device=None):
        ...
        previous_nhid = nfeat
        for current_nhid in nhid:
            self.layers.append(GraphConvolution(previous_nhid, current_nhid, with_bias=with_bias))
            previous_nhid = current_nhid

        self.layers.append(GraphConvolution(current_nhid, nclass, with_bias=with_bias))
    ...
    def _train_without_val(self, labels, idx_train, train_iters, verbose):
        ...
        # scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[2000, 4000, 6000, 8000, 10000], gamma=0.5)
        # scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[2000*x for x in range(1, 50)], gamma=0.5)
        scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[2000, 4000, 6000, 8000, 16000, 32000, 50000], gamma=0.5)
        ...
        for i in range(train_iters):
            ...
            if i % 1000 == 0:
                torch.save(self.state_dict(), 'ckp/mlgcn_%s_%s.pt' % (i, loss_train.item()))
    ...
    def drop_dissimilar_edges(self, features, adj, metric='dis'):
        ...
        # dropedge_cosine(adj_triu.data, adj_triu.indptr, adj_triu.indices, features, threshold)
        if metric == 'dis':
            dropedge_dis(adj_triu.data, adj_triu.indptr, adj_triu.indices, features, threshold=2.6)
        else:
            dropedge_cosine(adj_triu.data, adj_triu.indptr, adj_triu.indices, features, threshold=0.01)
...
@njit
def neighbor_degree(A, iA, jA, degree):
    res = np.zeros((len(iA)-1, 4))
    for row in range(len(iA)-1):
        res[row, 3] = 10000
        for i in range(iA[row], iA[row+1]):
            # print(row, jA[i], A[i])
            ...
            n2 = jA[i]
            res[n1, 1] += degree[n2]
            res[n1, 2] = max(res[n1, 2], degree[n2])
            res[n1, 3] = min(res[n1, 3], degree[n2])
        res[row, 0] = degree[row]
        res[row, 1] = res[row, 1] #/ (iA[row+1] - iA[row])
    return res
